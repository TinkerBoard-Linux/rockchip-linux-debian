...
int GET_DRV_OFFSET(int bank, int pin) {
    int grp = pin / 8;
    int DRV_TABLE_3B[4] = {GRF_GPIO3B_DS_0, GRF_GPIO3B_DS_1, GRF_GPIO3B_DS_2,
                           GRF_GPIO3B_DS_3};        //Bank 3
    int DRV_TABLE_4C[4] = {GRF_GPIO4C_DS_0, GRF_GPIO4C_DS_1, GRF_GPIO4C_DS_2,
                           GRF_GPIO4C_DS_3};        //Bank 3
...
void asus_set_pwmFrequency(int pin, int divisor) {
    ...
    if (asus_get_pin_mode(pin) == PWM && PWM_CTRL_OFFSET != -1) {
        *(pwm + PWM_CTRL_OFFSET) &= ~(1 << 0);        //Disable PWM
        *(pwm + PWM_CTRL_OFFSET) =
                (*(pwm + PWM_CTRL_OFFSET) & ~(0xff << 16)) | ((0xff & (divisor / 2)) << 16) |
                (1 << 9);        //PWM div
        *(pwm + PWM_CTRL_OFFSET) |= (1 << 0); //Enable PWM
    }
}
    ...
    if (alt == 0) {
        if (bank_pin < 16)
            if (*(gpio0[bank] + GPIO_SWPORT_DDR_L) & (1 << bank_pin))
                ...
                alt = FSEL_INPT;
        else if (*(gpio0[bank] + GPIO_SWPORT_DDR_H) & (1 << (bank_pin - 16)))
            alt = FSEL_OUTP;
        else
            alt = FSEL_INPT;
    }
...
void SetGpioMode(int pin, int alt) {
    ...
    switch (pin) {
        ...
        case GPIO3_B2 :
        case GPIO3_B3 :
            *(grf + GRF_GPIO3B_IOMUX_L) =
                    (*(grf + GRF_GPIO3B_IOMUX_L) | (0x07 << ((pin % 8) * 4 + 16)) |
                     (0x7 << ((pin % 8) * 4))) & (~(alt << ((pin % 8) * 4)));
            break;
        case GPIO3_B4 :
            *(grf + GRF_GPIO3B_IOMUX_H) =
                    (*(grf + GRF_GPIO3B_IOMUX_H) | (0x07 << ((pin % 8) * 4 + 16)) |
                     (0x7 << ((pin % 8) * 4))) & (~(alt << ((pin % 8) * 4)));
            break;
        ...
        case GPIO4_C2 :
        case GPIO4_C3 :
            *(grf + GRF_GPIO4C_IOMUX_L) =
                    (*(grf + GRF_GPIO4C_IOMUX_L) | (0x07 << ((pin % 8) * 4 + 16)) |
                     (0x7 << ((pin % 8) * 4))) & (~(alt << ((pin % 8) * 4)));
            break;
        case GPIO4_C4 :
        case GPIO4_C5 :
        case GPIO4_C6 :
            *(grf + GRF_GPIO4C_IOMUX_H) =
                    (*(grf + GRF_GPIO4C_IOMUX_H) | (0x07 << ((pin % 8) * 4 + 16)) |
                     (0x7 << ((pin % 8) * 4))) & (~(alt << ((pin % 8) * 4)));
            break;
