...
if [ "$?" == "0" ]; then
	...
	gst-launch-1.0 v4l2src device=/dev/video0 device=$CSI0 num-buffers=100 ! video/x-raw,format=NV12,width=1920,height=1080 ! videoconvert ! autovideosink
	gst-launch-1.0 v4l2src device=/dev/video0 device=$CSI0 num-buffers=10 ! video/x-raw,format=NV12,width=1920,height=1080 ! jpegenc ! multifilesink location=$PicPath
